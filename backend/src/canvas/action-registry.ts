...
export const ACTION_REGISTRY: RegisteredAction[] = [
  ...
  action({ name: 'run.migrate', description: '将内容一致的成功产物历史迁移到替代画布节点',
    machineDescription: 'Register a copied target-canvas asset as a migrated history item for an original succeeded run. Requires target canvas lease and verifies source/target asset ownership, media kind and SHA-256 content equality. Does not invoke a provider, mutate the source run, change graph revision, or replace current/approved selection.',
    method: 'POST', path: '/api/runs/:id/migrate', scope: 'run', requiresLease: true, idempotent: true,
    sideEffects: ['migration_record', 'candidate_append'], errors: [...canvasWriteErrors,
      { code: 'RUN_NOT_MIGRATABLE', status: 409, description: '原 Run 不是可迁移的原始成功记录' },
      { code: 'RUN_MIGRATION_ASSET_MISMATCH', status: 400, description: '来源或目标资产归属、节点、类型不匹配' },
      { code: 'RUN_MIGRATION_CONTENT_MISMATCH', status: 400, description: '来源与目标文件内容不一致' },
      { code: 'ASSET_ALREADY_RECORDED', status: 409, description: '目标资产已登记成功历史' }],
    inputSchema: { type: 'object', required: ['path', 'body'], properties: {
      path: { type: 'object', required: ['id'], properties: { id: { type: 'string', minLength: 1 } }, additionalProperties: false },
      body: { type: 'object', required: ['leaseToken', 'leaseEpoch', 'expectedRevision', 'targetCanvasId', 'targetNodeId', 'targetAssetId', 'reason'], properties: {
        leaseToken: { type: 'string', minLength: 1 }, leaseEpoch: { type: 'integer' }, expectedRevision: { type: 'integer' },
        targetCanvasId: { type: 'string', minLength: 1, maxLength: 100 }, targetNodeId: { type: 'string', minLength: 1, maxLength: 200 },
        targetAssetId: { type: 'string', minLength: 1, maxLength: 100 }, sourceAssetId: { type: 'string', minLength: 1, maxLength: 100 }, reason: { type: 'string', minLength: 1, maxLength: 2000 },
      }, additionalProperties: false },
    }, additionalProperties: false } }),
  ...
  ...[
    ...
    ['local_compute_scheduler.thermal_policy','GET','/api/local-compute-scheduler/thermal-policy'], ['local_compute_scheduler.thermal_policy_update','PUT','/api/local-compute-scheduler/thermal-policy'],
...
function ioMeaning(command: string) {
  ...
  if (command === 'output.clear') return '一次性清空输出区全部已发布项，新增一个空输出版本；已有下游快照不受影响，输出区为空时拒绝';
  if (command === 'input.capture') return '固定其他画布当前输出并复制为本地快照；可用 sourceItemKey + bindNodeId 原子选择单项并绑定已有空输入节点';
...
function commandSchema(lease: boolean, command?: string, fields?: Record<string, unknown>, requiredFields?: string[]) {
  const string = { type: 'string' }; const payload: Record<string, unknown> = {
    groupId: string, itemKey: string, snapshotId: string, sourceCanvasId: string, sourceItemKey: string, bindNodeId: string, sourceOutputsVersion: { type: 'integer' },
  ...
  const shape: Record<string, [string[], string[]]> = {
    'input.capture': [['sourceCanvasId','sourceOutputsVersion','sourceItemKey','bindNodeId','name'], ['sourceCanvasId']],
    ...
    'output.reorder': [['order'], ['order']], 'output.clear': [[], []], 'edit': [['name','description'], []],
