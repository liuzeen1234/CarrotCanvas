...
export const ACTION_REGISTRY: RegisteredAction[] = [
  ...
  action({ name: 'run.cancel', description: '请求取消 Run；当前 provider 均不支持真正的 run 级精确取消，活跃 Run 会返回 CANCEL_NOT_PRECISE', machineDescription: 'Inspect run.capabilities.cancel first. Never treat a global provider interrupt as precise cancellation.', method: 'POST', path: '/api/runs/:id/cancel', scope: 'run', idempotent: true, errors: [...standardErrors, { code: 'CANCEL_NOT_PRECISE', status: 409, description: '提供方不支持安全的 run 级精确取消' }] }),
  ...
  ...[
    ...
    ['gpu_scheduler.status','GET','/api/gpu-scheduler/status'],
