...
export const ACTION_REGISTRY: RegisteredAction[] = [
  ...
  action({ name: 'run.cancel', description: '请求取消 Run；TTS 在语音片段边界取消，speech-evaluator 在工具与音频边界取消', machineDescription: 'Inspect run.capabilities.cancel first. TTS stops at a speech-segment boundary; speech evaluation stops before opening the next audio unit. Completed evidence is preserved.', method: 'POST', path: '/api/runs/:id/cancel', scope: 'run', idempotent: true, errors: [...standardErrors, { code: 'CANCEL_NOT_PRECISE', status: 409, description: '提供方不支持安全的 run 级精确取消' }] }),
  action({ name: 'speech_evaluator.run.submit', description: '提交严格串行的语音评价批次并在完整生命周期内持有本机重型计算租约', method: 'POST', path: '/api/speech-evaluator/runs', scope: 'run', requiresLease: true, idempotent: true, sideEffects: ['generation_run', 'local_compute_lease', 'speech_evaluation_items'], errors: canvasWriteErrors }),
  action({ name: 'speech_evaluator.run.get', description: '读取语音评价批次及每个工具和音频的持久化阶段结果', method: 'GET', path: '/api/speech-evaluator/runs/:id', scope: 'run' }),
  action({ name: 'speech_evaluator.run.cancel', description: '在最近的工具与音频安全边界取消语音评价，保留已完成阶段结果', method: 'POST', path: '/api/speech-evaluator/runs/:id/cancel', scope: 'run', idempotent: true, sideEffects: ['generation_run', 'speech_evaluation_items'] }),
  ...
  ...[
    ...
    ['tts.providers','GET','/api/tts/providers'],
