...
export const ACTION_REGISTRY: RegisteredAction[] = [
  ...
  action({ name: 'run.cancel', description: '请求取消 Run；speech-evaluator 支持音频安全边界取消，其他 provider 可能返回 CANCEL_NOT_PRECISE', machineDescription: 'Inspect run.capabilities.cancel first. Speech evaluation stops before opening the next audio unit and preserves completed evidence.', method: 'POST', path: '/api/runs/:id/cancel', scope: 'run', idempotent: true, errors: [...standardErrors, { code: 'CANCEL_NOT_PRECISE', status: 409, description: '提供方不支持安全的 run 级精确取消' }] }),
  ...
  ...[
    ...
    ['tts.providers','GET','/api/tts/providers'], ['tts.run.submit','POST','/api/tts/runs'],
