...
export class CanvasService implements OnModuleInit {
  ...
  async agentView(id: string) { const canvas = await this.findOne(id); const io = canvas.io ?? emptyCanvasIo(); return { canvas,
    ioSummary: { inputGroups: io.inputs.map(group => ({ groupId: group.id, name: group.name, sourceType: group.sourceType, sourceCanvasId: group.sourceCanvasId, activeSnapshotId: group.activeSnapshotId, items: activeInput(group).items.map(item => ({ itemKey: item.itemKey, assetId: item.assetId, name: item.name, kind: item.kind })) })), outputsVersion: io.outputs.at(-1)?.version ?? 0, outputs: (io.outputs.at(-1)?.items ?? []).map(item => ({ itemKey: item.itemKey, assetId: item.assetId, name: item.name, kind: item.kind, sourceNodeId: item.sourceNodeId, outputSlot: item.outputSlot })), retainedWorkingInputs: canvas.graph.nodes.filter(node => node.data.inputDetachedReason).map(node => ({ nodeId: node.id, inputGroupId: node.data.inputGroupId, snapshotId: node.data.inputSnapshotId, itemKey: node.data.inputItemKey, assetId: node.data.inputLocalAssetId, reason: node.data.inputDetachedReason })) }, ioImportUrl: `/api/canvas/${id}/io/files`, projectsUrl: '/api/projects', projects: (await this.list()).find(c => c.id === id)?.projects ?? [], control: await this.controlStatus(id), supportedOperations: ['replace_graph', 'rename_canvas', 'set_brief', 'create_node', 'update_node', 'move_nodes', 'delete_node', 'connect', 'disconnect'], actionsUrl: '/api/actions', ioUrl: `/api/canvas/${id}/io`, ioCommandUrl: `/api/canvas/${id}/io/command`, operationLogUrl: `/api/canvas/${id}/operation-log`, checkpointsUrl: `/api/canvas/${id}/checkpoints` }; }
