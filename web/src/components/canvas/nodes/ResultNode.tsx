  ...
  return <div className={`canvas-node canvas-node--result${props.selected ? ' selected' : ''}`}>
    ...
    <div className="canvas-node__body canvas-node__result-body nodrag">
      ...
      {running ? <div style={{ width: '100%', textAlign: 'center' }}><Spin /><div style={{ margin: '8px 0', color: '#888' }}>{run?.status === 'pending' ? '等待 ComfyUI 执行…' : run?.currentNodeTitle || '正在生成…'}</div><Progress size="small" percent={percent} status="active" showInfo={percent !== undefined} /></div>
        ...
        : kind === 'audio' && audios.length ? <Space direction="vertical" size={8} style={{ width: '100%' }}>{audios.map((asset) => <div key={asset.assetId} style={{ width: '100%' }}><ViewportAudio controls preload="metadata" src={asset.url} style={{ width: '100%' }} /><AssetIdLabel assetId={asset.assetId} /><PublishOutputButton payload={{ assetId: asset.assetId, name: asset.filename }} /><Button size="small" icon={<DownloadOutlined />} href={`/api/assets/${asset.assetId}/download`} download={asset.filename || ''} onClick={(event) => event.stopPropagation()}>下载</Button></div>)}</Space>
        : previewItems.length ? <Space direction="vertical" size={8} style={{ width: '100%' }}>{previewItems.map((asset, assetIndex) => <div key={asset.assetId} className="canvas-result-image"><button type="button" className="canvas-media-trigger" onClick={() => setPreviewIndex(assetIndex)} aria-label={`放大预览${asset.kind === 'video' ? '视频' : '图片'}`}>{asset.kind === 'video' ? <><ViewportVideo src={asset.url} muted playsInline preload="metadata" /><PlayCircleFilled className="canvas-media-trigger__play" /></> : <ViewportImage loading="lazy" decoding="async" src={asset.url} alt={asset.filename || '生成图片'} />}</button><AssetIdLabel assetId={asset.assetId} /><PublishOutputButton payload={{ assetId: asset.assetId, name: asset.filename }} /><Button size="small" icon={<DownloadOutlined />} href={`/api/assets/${asset.assetId}/download`} download={asset.filename || (asset.kind === 'video' ? 'canvas-video.mp4' : 'canvas-image.png')} onClick={(event) => event.stopPropagation()}>下载</Button></div>)}</Space>
