...
export default function ReferenceInputNode({ id, data }: NodeProps) {
  ...
  return <div className="canvas-node canvas-node--result">
    ...
    <div className="canvas-node__body nodrag">
      ...
      {data.inputGroupId ? <div style={{ fontSize: 12, color: '#888', marginBottom: 8 }}>输入快照 · 在输入区更新内容</div> : null}
      {kind === 'text' ? <ImeSafeTextArea value={String(data.lastText ?? '')} disabled={readOnly || !!data.inputGroupId} onChange={lastText => updateNodeData(id,{lastText})} autoSize={{minRows:3}} placeholder="提示词、数字、true / false 或下拉选项值" /> : <>
        {asset ? kind === 'video' ? <ViewportVideo src={asset.url} controls style={{width:'100%'}} /> : kind === 'audio' ? <ViewportAudio src={asset.url} controls style={{width:'100%'}} /> : <ViewportImage loading="lazy" decoding="async" src={asset.url} alt={asset.filename} style={{width:'100%'}} /> : null}
