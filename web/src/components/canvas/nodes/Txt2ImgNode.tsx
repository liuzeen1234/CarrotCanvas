...
export default function Txt2ImgNode(props: NodeProps) {
  ...
  return <div className={`canvas-node canvas-node--txt2img${props.selected ? ' selected' : ''}${missingBindings.length ? ' canvas-node--reference-error' : ''}`}>
    ...
    <div className="canvas-node__body canvas-node__form-body nodrag">
      ...
      {(data.lastAssets ?? []).length ? <Space direction="vertical" size={6} style={{ width: '100%', marginTop: 8 }}>{(data.lastAssets ?? []).map((asset, assetIndex) => <div key={asset.assetId}>{asset.kind === 'audio' ? <audio controls preload="metadata" src={asset.url} style={{ width: '100%' }} /> : <button type="button" className="canvas-media-trigger" onClick={() => setPreviewIndex(assetIndex)} aria-label={`放大预览${asset.kind === 'video' ? '视频' : '图片'}`}>{asset.kind === 'video' ? <><video src={asset.url} muted playsInline preload="metadata" /><PlayCircleFilled className="canvas-media-trigger__play" /></> : <img loading="lazy" decoding="async" src={asset.url} alt={asset.filename || '生成图片'} />}</button>}<AssetIdLabel assetId={asset.assetId} /><Button size="small" block icon={<DownloadOutlined />} href={`/api/assets/${asset.assetId}/download`} download>下载</Button></div>)}</Space> : null}
