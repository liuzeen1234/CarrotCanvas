...
export default function CodexCapabilityNode(props: NodeProps) {
  ...
  const { canvasId, updateNodeData, deleteNode, getUpstreamAsset, getUpstreamText } = useContext(CanvasNodeDataContext);
  ...
  const run = async () => {
    ...
    try {
      if (data.capability === 'text') {
        ...
        const result = await postJson<CodexImageResponse>('/api/codex2api/images/generations', { prompt: effectivePrompt, model: data.model || 'codex', n: 1, size: data.size, response_format: data.responseFormat, canvasId, nodeId: props.id }, controller.signal);
  ...
  return <div className={`canvas-node canvas-node--codex${props.selected ? ' selected' : ''}`}>
    ...
    <div className="canvas-node__body nodrag"><Space direction="vertical" size={8} style={{ width: '100%' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <ImeSafeTextArea autoSize={{ minRows: 3, maxRows: 8 }} value={upstreamPrompt.connected ? upstreamPrompt.text : data.prompt} onChange={(prompt) => update({ prompt })} disabled={upstreamPrompt.connected} placeholder={upstreamPrompt.connected ? '等待上游输出文本' : '输入提示词'} />
      </div>
