...
import { ViewportImage } from '../ViewportMedia';
    ...
    <div className="canvas-node__body nodrag"><Space direction="vertical" size={8} style={{ width: '100%' }}>
      ...
      {needsImage ? <>
        ...
        <div className="canvas-reference-grid">{references.map((item, index) => <div key={item.referenceId} className="canvas-reference-thumb" draggable={!readOnly} onDragStart={(event) => event.dataTransfer.setData('text/reference-id', item.referenceId)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); moveReference(event.dataTransfer.getData('text/reference-id'), item.referenceId); }}>
          <button type="button" className="canvas-reference-thumb__preview" onClick={() => setPreview({ kind: 'reference', index })} aria-label={`预览第 ${index + 1} 张参考图`}><ViewportImage loading="lazy" decoding="async" src={item.url} alt={item.displayName} /><span>{index + 1}</span></button>
      ...
      {displayedText ? <><div className="canvas-codex-text">{displayedText}</div><Space wrap><PublishOutputButton disabled={busy || displayedText !== data.lastText} payload={{ nodeId: props.id, name: data.cardName || '文字输出' }} />{data.lastTextParts?.positive && <PublishOutputButton disabled={busy} label="正向设为输出" payload={{ nodeId: props.id, textPart: 'positive', name: '正向提示词' }} />}{data.lastTextParts?.negative && <PublishOutputButton disabled={busy} label="负向设为输出" payload={{ nodeId: props.id, textPart: 'negative', name: '负向提示词' }} />}</Space></> : null}
      {images.map((item, index) => <div key={`${item.url}-${index}`}><button type="button" className="canvas-media-trigger" onClick={() => setPreview({ kind: 'output', index })} aria-label="放大预览图片"><ViewportImage loading="lazy" decoding="async" src={item.url} alt="生成图片" /></button><AssetIdLabel assetId={item.assetId} /><PublishOutputButton payload={{ nodeId: props.id, assetId: item.assetId, name: data.cardName || '图片输出' }} disabled={!item.assetId} /><Button size="small" block icon={<DownloadOutlined />} href={item.assetId ? `/api/assets/${item.assetId}/download` : item.url} download>下载</Button></div>)}
      <NodeOutputHistory cardName={data.cardName} currentAssetId={data.lastAssets?.[0]?.assetId} canvasId={canvasId} nodeId={props.id} kind={outputKind} promptModeContext={data.capability} readOnly={readOnly} control={control} refreshKey={`${historyVersion}:${generationHistoryVersion}`} onSelectAsset={(asset) => update({ lastAssets: [asset] })} onSelectText={(text, parts) => { setLiveText(''); update({ lastText: text, lastTextParts: parts || undefined }); }} onObserveAsset={(asset) => observeNodeData(props.id, { lastAssets: [asset] })} onObserveText={(text, parts) => { setLiveText(''); observeNodeData(props.id, { lastText: text, lastTextParts: parts || undefined }); }} />
    </Space></div>
