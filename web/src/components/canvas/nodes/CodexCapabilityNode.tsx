...
import { capabilityPromptHandle, CodexCapabilityNodeData, resultSourceHandle, resultTargetHandle } from './types';
...
export default function CodexCapabilityNode(props: NodeProps) {
  ...
  const run = async () => {
    ...
    try {
      if (data.capability === 'text') {
        const body = { model: data.model || 'codex', messages: [{ role: 'user', content: effectivePrompt }] };
        let text = '';
        ...
        const result = await postJson<CodexImageResponse>('/api/codex2api/images/generations', { prompt: effectivePrompt, model: data.model || 'codex', n: 1, size: data.size, response_format: data.responseFormat, canvasId, nodeId: props.id, ...control }, controller.signal);
        update({ lastAssets: result.data.map((item) => ({ assetId: item.assetId || '', url: imageSource(item), kind: 'image' })) });
      } else {
        const form = new FormData(); form.append('image', await imageFile()); form.append('prompt', effectivePrompt); form.append('model', data.model || 'codex');
        if (data.capability === 'edit') { form.append('n', '1'); form.append('size', data.size || '1024x1024'); form.append('response_format', data.responseFormat || 'url'); if (canvasId) form.append('canvasId', canvasId); form.append('nodeId', props.id); if (control) { form.append('leaseToken', control.leaseToken); form.append('leaseEpoch', String(control.leaseEpoch)); form.append('expectedRevision', String(control.expectedRevision)); } const result = await postForm<CodexImageResponse>('/api/codex2api/images/edits', form, controller.signal); update({ lastAssets: result.data.map((item) => ({ assetId: item.assetId || '', url: imageSource(item), kind: 'image' })) }); }
  ...
  return <div className={`canvas-node canvas-node--codex${props.selected ? ' selected' : ''}`}>
    ...
    <div className="canvas-node__body nodrag"><Space direction="vertical" size={8} style={{ width: '100%' }}>
      ...
      {data.capability === 'text' ? <Switch disabled={readOnly} size="small" checked={data.stream !== false} checkedChildren="流式" unCheckedChildren="普通" onChange={(stream) => update({ stream })} /> : null}
