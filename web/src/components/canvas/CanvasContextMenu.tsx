...
export default function CanvasContextMenu({ state, onClose, onPick, onPickCapability, onPickTts, onPickSpeechEvaluator, onPickInput }: CanvasContextMenuProps) {
  ...
  const items: MenuProps['items'] = useMemo(() => {
    ...
    const ttsItems: MenuProps['items'] = !state?.connection || ['audio', 'text'].includes(state.connection.kind) ? [{ key: 'tts', label: 'AI 配音（本地）' }, { key: 'speech-evaluator', label: 'AI 语音评价' }, { type: 'divider' }] : [];
  ...
  const handleClick: MenuProps['onClick'] = ({ key }) => {
    ...
    if (key === 'speech-evaluator') { onPickSpeechEvaluator(); onClose(); return; }
