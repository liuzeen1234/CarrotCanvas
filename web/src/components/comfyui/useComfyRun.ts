...
export function useComfyRun(args: UseComfyRunArgs) {
   ...
   */
  const submit = async (apiJsonOverride?: unknown, connectedKeys: ReadonlySet<string> = new Set(), inputAssetIds: string[] = [], referenceInput?: { originalPrompt: string; imageReferenceMap?: Array<Record<string, unknown>>; referenceMaps?: Record<string, Array<Record<string, unknown>>> }): Promise<boolean> => {
    const w = workflowRef.current;
    if (!w) return false;
    let apiJson: unknown;
    try {
