...
> 2026-09-15 端口对齐修复：统一为 `.canvas-node` 设置相对定位，使输入/输出 Handle 以可见卡片而非持久化尺寸的 React Flow 外层定位。真实「动态测试」画布计划卡外层宽 700、卡片宽 500，修复前在 0.3 缩放下右端点偏离 60px；修复后六张卡片横向仅为边框内侧 0.33px、纵向偏差 0，刷新和平移复验通过。不修改 graph 或卡片内容。

> 2026-09-15 标题复制：画布工作台卡片标题与画布内无编辑权限的悬浮标题均在最右侧提供独立复制按钮，长标题单行截断且悬停可查看全名；可编辑时继续双击重命名。复制提供成功/失败提示。安全上下文优先 Clipboard API，普通 HTTP 在点击事件中同步使用 textarea + execCommand('copy') 回退，并恢复焦点与选区。前端生产构建及 localhost 两处成功提示验收通过；内置浏览器虚拟剪贴板无法核对系统复制内容，局域网 HTTP 页面访问超时，未完成真实非安全上下文的端到端验收。传统复制接口已弃用，兼容性依浏览器而定。

> 2026-09-14 项目与画布 IO：项目多对多组织画布；编辑器增加输入/工作/输出三区域，四类本地文件复制导入、明确发布输出、任意跨画布整组快照、主动更新和历史回退；项目成果为关联输出的独立浏览快照，不可被引用。IO 接入既有 revision/lease/日志/Checkpoint，Run 冻结输入来源。22 suites / 185 tests、生产构建、页面与重启验收通过；旧画布和历史逐字段对比保持原值。详见 PROJECTS-AND-CANVAS-IO.md。

> 2026-09-13 画布媒体可见范围加载：结果、生成输出、参考输入及节点历史统一使用共享 IntersectionObserver（160px 预加载边距），离屏移除 src，音视频暂停并 load 释放缓冲，保留已显示媒体高度及播放位置；节点继续挂载，运行与历史同步不受影响。真实「许明短片 · 对话版人物选角」画布 53 节点、145 个媒体预览，采样视野仅 12 个带 src、133 个离屏无 src、视频 src 为 0；平移与返回恢复通过，修正参考图卸载高度变化后复验 3 个卸载媒体尺寸均不变。前端生产构建和新增组件严格类型检查通过；全量 tsc 仍存在仓库已有 Umi 解析/其他节点类型错误，尚未量化拖动帧率，不能视为已证明唯一瓶颈。

> 2026-09-13 Run 补录恢复：生成卡片与画布流水可显式登记已有媒体资产为独立恢复历史，保留原失败记录、当前/批准产物与 graph revision。记录原 Run、原因、来源说明、操作者与补录时间，不重新生成；支持幂等及事务回滚，21 suites / 169 tests 通过。详见 RUN-RECOVERY.md。

> 2026-09-13 大画布性能：节点位置/选择/尺寸更新不再使共享内容 Context 全量刷新，节点组件 memo；引用保护扫描只在删除时执行，拖拽中间帧跳过 graph 序列化，松手仍按原流程自动保存；卡片图片 lazy/async 加载。保留离屏节点挂载，避免中断运行轮询或中文输入；尚未量化实际帧率与内存改善。

> 2026-09-12 Stable Audio 3：新增本地普通 Medium 音效工作流及 `txt2audio` 画布入口，提示词/时长/seed/可选本地 Qwen 扩写与官方扩写类别表单，默认 FLAC；沿用统一调度、Run、资产与候选历史，不改已有图像/视频工作流。部署、真实验收证据及需人工试听的边界见 `STABLE-AUDIO-INTEGRATION.md`。

> 2026-09-12 增量：已启用托管配置时，CarrotCanvas 后端启动会异步检查并经统一调度器预热 ComfyUI；已在线的托管实例会重新登记为 resident。跨 Provider 切换优先调用 `/free` 卸载模型并验证 CUDA allocator，成功后保留 8188 API 服务，验证失败才回退为结束托管进程树。
...
> 2026-09-04 增量：Codex2API 画布能力卡片的提示词支持 `text` 连线输入；文生文/图像理解结果可继续连接到四类能力的提示词，连接值覆盖本地值、断开回退，并限制单一输入来源与环路。
> 2026-09-04 增量：ComfyUI 工作流的 STRING 字段也可在导入/编辑阶段配置为 `text` 输入；画布按字段显示文本端口，并在运行时以上游完整文本覆盖对应参数。
...
> 2026-09-06 图片提示词反推：图像理解卡片将 `image-prompts` 明确呈现为“图片提示词反推”，使用视觉复现专用指令并记录 `reverse-image-prompt` Run 意图；保持旧历史与合并/正向/负向输出兼容。
> 2026-09-06 画布交互与耗时：新增默认手掌、指针框选/多选拖动模式；ComfyUI 与 Codex2API 卡片显示运行中耗时，节点历史和全局历史显示持久 Run 的实际生成耗时与时间明细。
...
> 2026-09-11 Codex2API 多图引用：图生图/图像理解单图片端口支持最多 16 条入线及持久上传，卡片提供计数、有序缩略图、放大、拖动排序和移除；结构化 `@` 引用以稳定 referenceId 绑定图片并在提交前按当前顺序编译，前后端统一阻止删除仍被引用的图片来源。Run 保存原始/实际提示词和图片顺序映射；沿用 graph JSON 与 inputSnapshot，无数据库迁移。
...
│  │  │  └─ comfyui.module.ts
│  │  ├─ canvas/                 # Canvas + AI Native 控制（revision/lease/operations/log/checkpoint/Agent lease guard）
│  │  │  ├─ canvas.entity.ts
...
- ✅ ComfyUI、CosyVoice 3、IndexTTS2、Qwen3-TTS 共用 SQLite FIFO 本机重型计算租约；正式生成在队首执行默认 `≤50°C` 的 GPU 温控检查（每分钟、最多 10 轮，超时中止当时等待批次），并提供 `/api/local-compute-scheduler/status`、持久温控策略接口（旧 GPU 路径兼容）与各 Provider 可观测状态。
