...
> 2026-09-15 标题复制：画布工作台卡片标题单击复制完整名称并阻止卡片跳转；画布内无编辑权限时，悬浮标题区域单击复制，可编辑时继续双击重命名。两处均支持 Enter/空格，提供成功/失败提示。安全上下文优先 Clipboard API，普通 HTTP 在点击事件中同步使用 textarea + execCommand('copy') 回退，并恢复焦点与选区。前端生产构建及 localhost 两处成功提示验收通过；内置浏览器虚拟剪贴板无法核对系统复制内容，局域网 HTTP 页面访问超时，未完成真实非安全上下文的端到端验收。传统复制接口已弃用，兼容性依浏览器而定。
