...
- [x] C6 前端：节点内运行——**点运行先按 schema 做参数完整性校验（缺项标红不提交）→ 校验通过若未连结果节点则自动创建并连线（§4.3.1）→ 提交 / 轮询 / 中断 / 状态** + 结果走平台资产 URL 展示；工作流缺失时历史结果仍可见。已落地：schema 增 `required` 标记；生成节点接入共享 `useComfyRun`，携带 `canvasId/nodeId` 提交，支持校验红框、自动连出/复用结果节点、排队/进度/错误/中断；结果节点按连线读取上游瞬时运行态与 `lastAssets`，支持平台图片预览、放大、下载；删生成节点同步调用资产清理端点。后端成功状态延迟到资产捕获完成后发布，保证终态输出已有 `assetId/assetUrl`。后端测试 35 项全绿，前后端构建通过
- [x] C7 前端：画布防抖自动保存 + 视口持久化 + 刷新恢复（关掉 ComfyUI 也能看历史产物）——已落地：节点/连线/视口变化 800ms 防抖，保存请求严格串行且变更期间追写最新快照，剔除 selected/dragging/measured 等瞬时 UI 字段；顶栏展示待保存/保存中/已保存/保存失败与最新时间。实机完整验收通过：节点、参数、精确 viewport、自动结果连线与 `lastAssets` 均写入 graph；刷新后恢复；ComfyUI 8188 确认离线时刷新仍可见平台历史图，随后已恢复服务；临时第二画布真实生成资产与主画布互不串，删临时工作流后资产仍 200，删临时画布后其资产目录消失。前端 build 通过
...
- 2026-09-06：增加手掌/指针两种画布交互模式。手掌负责平移和单节点拖动；指针负责部分相交框选、多选整体拖动及空白取消选择，且支持在非输入区域按住空格临时切换为手掌平移。模式与选择态不进入持久图，节点移动继续复用 canonical canvas 操作。
- 2026-09-06：卡片运行态与历史产物增加耗时展示。运行中计时由持久 Run 时间戳驱动并在前端本地刷新，节点历史和全局历史统一展示实际生成耗时及排队/总耗时详情。

- 2026-09-03（实机验收）：新建“文生图转视频链路验收”画布，按真实 UI 完整跑通：右键创建 `Z-Image文生图` → 512×512、8 步生成图片资产 `zimage_2k_16x9_00009_.png` → 从图片结果卡输出端点拖至空白处 → 兼容菜单正确禁用不支持 image 输入的分类并展示图生图/图生视频 → 选择 `MiniMax H3 高质量图生视频` 后自动建节点并连接 `LoadImage.image`（端点变绿）→ 填写运镜提示词并以 8 步成功生成 `MiniMax_H3_00014_.mp4`。最终画布 4 节点/3 连线；视频结果卡浏览器实测 `readyState=4`、5.167 秒、864×480、无解码错误。

- 2026-09-03（实现）：支持从结果卡/生成节点的类型输出端点拖线到画布空白处，松开后自动弹出工作流菜单；菜单按输出 kind 过滤，只展示声明了同类型可连接输入的工作流。选择后在放开位置创建工作流节点，并自动连接到首个兼容输入字段，实现“图片结果 → 拖线 → 选择图生视频 → 自动接图”的快捷编排。

- 2026-09-03（缺陷修复）：画布视频实跑发现平台资产读取接口忽略浏览器 `Range` 请求，导致 MP4 在部分浏览器（尤其移动 Safari）无法正常起播或拖动。`GET /api/assets/:id` 现支持标准单段 byte range，返回 `206`、`Accept-Ranges`、`Content-Range` 与准确 `Content-Length`；完整读取仍为 200。新增视频分段响应测试。

- 2026-09-03（实现与验收）：画布开放 `txt2vid` / `img2vid` 右键分类，生成节点根据工作流分类提供 image/video 类型输出端点，自动结果节点同步采用对应类型输入/输出端点并对视频使用原生播放器预览、下载；连线继续执行同类型校验。浏览器实测从空画布成功创建 MiniMax H3 文生视频与图生视频节点，页面出现 2 个 video 输出端点，图生视频同时保留 1 个 image 输入端点。

- 2026-09-03（实现）：落地工作流卡片首个跨节点数据流闭环 `image → image`。工作流新增 `input_config`，导入/编辑弹窗在图片字段旁提供“允许连线”勾选；画布右键菜单开放图生图，工作流节点按实际分类显示；配置为可连接的图片字段显示带类型输入端点，结果卡显示图片输出端点。下游运行时优先解析上游平台资产，经 `/api/comfyui/upload/asset` 校验画布归属与类型后回灌 ComfyUI input，再覆盖目标字段提交。实机用无效手动文件名验证连线覆盖成功并生成/捕获新资产。端点结构已预留 video/audio/text，当前 UI 仅开放 image。
- 2026-09-04（实现）：开放 ComfyUI 工作流 `text → STRING` 字段连线。导入预览会把可编辑的 STRING 单行/多行字段与图片字段一起建议为可连接输入，导入和编辑界面可逐字段勾选；画布仅为 `input_config` 声明为 `text` 的字段渲染文本端口，运行时以上游节点最后一次完整 `lastText` 覆盖该字段。旧图片端口标识保持兼容。

- 2026-09-03（缺陷修复）：**修复中文输出文件名导致画布结果图破图**——三视图工作流产物名 `Z-Image-文生图_*.png` 含中文，资产读取/下载接口此前把它直接写入 `Content-Disposition: filename="..."`，Node 抛 `ERR_INVALID_CHAR` 并返回 500；纯 ASCII 文件名的普通文生图未触发。现统一生成 ASCII fallback `filename` + RFC 5987 UTF-8 `filename*`，内联展示与下载均保留兼容性。新增两项中文文件名 e2e 测试，后端 37 项全绿；重启 3100 后复验既有三视图资产 inline/download 均为 200，画布 `<img>` 完整加载为 1280×720，无需重新生成。

- 2026-09-03（实现与验收）：**C7 完成，Canvas 阶段二一期闭环可交付**——编辑器新增 graph 自动保存队列：节点、连线、表单/`lastAssets` 与视口变化 800ms 防抖，单请求串行，保存期间产生的新快照在完成后立即追写，避免响应乱序覆盖；持久化前移除 React Flow 的选中、拖动、尺寸测量瞬时字段；顶部增加保存状态与更新时间。实机验证：① 表单提示词与 512×512 参数、viewport transform 刷新前后精确一致；② 真实出图后 graph 为 2 节点/1 连线且含平台 asset 引用，刷新恢复图片；③ 停止 ComfyUI 后确认 8188 不可达、平台 asset 仍 200，浏览器刷新仍完整显示，之后按原命令恢复 ComfyUI 并确认 8188=200；④ 创建临时第二画布及临时工作流真实运行，主/次资产均 200、互不串；删除临时工作流后历史资产仍 200；删除临时画布后对应分区目录消失。验收临时画布/工作流已清理。

- 2026-09-03（实机验收）：**C6 真实 ComfyUI 端到端验证通过**——本机 ComfyUI 0.33.4（8188）、后端 3100、前端 8000：从空画布右键添加「Z-Image文生图」节点 → 点运行后自动创建并连出唯一结果节点，生成/结果两端同步显示节点进度 → 真实生成 `zimage_2k_16x9_00003_.png`，平台资产 HTTP 200、落盘文件 2,789,576 bytes → 同节点重跑仍保持 2 个节点，生成新 assetId，旧 asset URL 返回 404 且分区只留 1 个最新文件；清空必填主提示词再运行被前端拦截，显示 1 个缺失项，后端 run 数不增加且历史结果不受影响。因 C7 尚未实现，本轮临时节点 graph 不持久化（刷新后不恢复），符合当前分期边界。

- 2026-09-03（实现）：**C6 画布节点内运行闭环落地**——schema 字段补 `required` 来源信息；生成节点复用 `useComfyRun` 完成必填校验（缺项红框且不提交）、自动创建/复用结果节点、携带画布上下文提交、轮询进度与中断、成功后将平台资产引用写回 `lastAssets`；运行态通过 Canvas Context 独立保存在编辑器内存，不污染 graph。结果节点沿连线读取上游运行态与资产，展示 loading/错误/历史图片并支持放大下载；工作流缺失不影响历史资产展示；删除生成节点新增后端清理入口。修正 C2 捕获完成时序：画布运行等待资产落盘并回填 URL 后才进入 success。后端 35 项测试、前后端 build 均通过，3100 后端已按标准方式重启并验证健康/schema/清理路由。
