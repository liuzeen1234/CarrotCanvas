...
- 2026-09-18（候选组按 kind 默认选中）：修复一次 Run 同时产出多 kind（H3 视频卡的视频 + 独立音频）时，候选组只用单一 `selectedAssetId=ids[0]` 记录当前，导致输出数组首位常为音频、视频轨道显示未选中的问题。`GenerationCandidateGroup` 新增 `selected_by_kind`（`{image?,video?,audio?}`），`appendCandidates` 对本次产出的每个 kind 各自更新为最新资产，`selectedAssetId` 保留为主 kind（video > image > audio）向后兼容；手动 `choose` 只更新被选资产所属 kind，其余轨道保持不变。前端 `NodeOutputHistory` 按本轨道 kind（`selectedByKind[kind]`，缺失时回退到属于本 kind 的旧 `selectedAssetId`）判定“当前”，视频与音频轨道各自默认选中最新产物；`editor.tsx` 轮询签名纳入 `selectedByKind`。一次性迁移脚本 `artifacts/fix-selected-by-kind.cjs`（先备份 SQLite）回填 159 组历史候选，跳过 16 组。发布输出仍为按 slot 的单 asset，不改 Provider Run、租约或 revision，Phase 0A–1B 状态不变。runs.service 9 tests（含新增 per-kind 用例）通过，后端 `tsc -p tsconfig.build.json` 与前端生产构建通过，3100 重启健康、8000 可访问，API 复核 `experiment2-h3-role-01` 返回 `selectedByKind={audio,video}`。
