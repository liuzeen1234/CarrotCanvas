...
- 文生文与图像理解卡片均可选择“普通文本”或“图像提示词（正负分开）”。文生文的图像提示词模式由后端仅在该次请求中注入结构化输出 system message 并强制非流式；图像理解端点没有 chat message 结构，因此把同一结构化约束合并进本次分析指令。两者都把正向、负向提示词分别持久化；普通文本模式不增加结构化约束，行为保持不变。
